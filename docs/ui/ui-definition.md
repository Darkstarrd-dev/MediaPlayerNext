...
- 设置面板当前固定为双分页：`界面设置` / `数据库管理`
- 设置面板当前已开放 `界面设置` 与 `数据库管理` 两个分页；其中界面设置页首轮包含遮罩透明度、容器外边界系数、容器内边距系数、分割条宽度系数与容器内上中下间距系数五项运行时设置
...
- `清除数据库` 的最终确认已从原生确认框改为走 `4 小面板层`
- 点击设置页中的 `清除数据库` 现在只会打开确认小面板，不会提前执行任何清理动作
...
- `Sidebar.main` 当前已切到“路径节点 + 媒体源节点”语义：父路径节点运行时还原，媒体源节点来自 `media_sources`
- `Sidebar.main` 当前节点类型固定为 `folder / media_source`，点击 `folder` 仅更新选中态，点击 `media_source` 才驱动 `Main` 列表刷新
